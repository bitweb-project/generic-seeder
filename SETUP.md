...
2. **ONLY COMPLETE THIS STEP IF YOU ARE SETTING UP THE SEEDER FOR USE WITH CLOUDFLARE, OTHERWISE YOU MAY SKIP THIS STEP.**<br /><br />
   Extra setup is required before Cloudflare mode will work properly. Python 3.3+ and the Cloudflare Python API must be installed in a virtual environment. Run the following cmds in the terminal, one line at a time:

```
sudo apt-get install python3 python3-pip python3-venv
```

   The `cloudflare` Python library must be installed inside a virtual environment (venv). **Do not use `sudo pip3 install cloudflare` globally** — Ubuntu 24+ blocks this, and mixing system/global packages causes version conflicts. After cloning the repo in step 3 below, run:
...
```
cf-venv/bin/python3 -c "import cloudflare; print('OK')"
```
...
- **cf_domain (Optional):** The top-level domain name configured in Cloudflare. **Ex:** `bitwebcore.net`. Required if you followed the [Cloudflare Setup](#cloudflare-setup).
- **cf_domain_prefix (Optional):** The subdomain prefix for DNS records managed by the seeder. **Ex:** `seed3` will create records under `seed3.bitwebcore.net`. Required if you followed the [Cloudflare Setup](#cloudflare-setup).
- **cf_api_key (Optional):** A Cloudflare **API Token** (not the Global API Key) with permission to edit DNS records for your zone. To create one: log into Cloudflare → click your domain → **Overview** → scroll down to **API** → click **Get your API token** → **Create Token** → use the **"Edit zone DNS"** template → scope it to your domain → click **Create Token** and copy the result. **Ex:** `abc123xyz_your_token_here`. Required if you followed the [Cloudflare Setup](#cloudflare-setup).
- **cf_seed_dump (Optional):** The name of the dump file with the list of "good" nodes that the Cloudflare script should look for. Always set to `dnsseed.dump`. Required if you followed the [Cloudflare Setup](#cloudflare-setup).
...
10. **ONLY COMPLETE THIS STEP IF YOU ARE SETTING UP THE SEEDER FOR USE WITH CLOUDFLARE, OTHERWISE YOU MAY SKIP THIS STEP.**<br /><br />
   An extra step is required for taking the "good" list of nodes from the DNS seeder app and sending it to Cloudflare. To do this manually you can run the following cmd in the terminal (replace `/path/to/seeder/` with the actual path where you cloned the repo):<br /><br />
   `cd /path/to/seeder/cf-uploader && /path/to/seeder/cf-venv/bin/python3 seeder.py`<br /><br />
   **Important:** always use the venv Python binary (`cf-venv/bin/python3`), not the system `python3`. The system Python has the old cloudflare v2 package which is incompatible with the current code.<br /><br />
   After running this cmd, if everything went well you should be able to run the following cmd and see results (replace `seed3.bitwebcore.net` with your fully qualified NS record):<br /><br />
   `nslookup seed3.bitwebcore.net`<br /><br />
   The IP addresses returned are coming from your `dnsseed.dump` file. You can cross-reference them there.<br /><br />
   Once you are sure that everything is working, set up a cron job to automatically update the seeds list every 30 minutes:<br /><br />
   `crontab -e`<br /><br />
   Then add this line to the crontab (replace `/path/to/seeder/` with your actual path):<br /><br />
   `*/30 * * * * cd /path/to/seeder/cf-uploader && /path/to/seeder/cf-venv/bin/python3 seeder.py >> /path/to/seeder/cf-uploader/seeder.log 2>&1`<br /><br />
   Check the log after the first run:<br /><br />
   `tail -f /path/to/seeder/cf-uploader/seeder.log`
...
Below is a complete working example for **Bitweb Core** (`bitwebcore.net`), using the Cloudflare method with `seed3.bitwebcore.net` as the NS record. All values are taken from the Bitweb Core source — `src/node/protocol_version.h` for protocol versions and `src/chainparams.cpp` for magic bytes and port.

### settings.conf (Bitweb Core)
...
pchMessageStart_0 = "0xfe"
pchMessageStart_1 = "0xae"
pchMessageStart_2 = "0xd5"
pchMessageStart_3 = "0xca"
wallet_port="26333"
explorer_url="https://explorer.bitwebcore.net/api/getblockcount"
second_explorer_url=""
explorer_requery_seconds="60"
block_count="60000"
seed_1="127.0.0.1"
seed_2="173.212.243.200"
seed_3="185.32.162.191"
seed_4="seed1.bitwebcore.net"
seed_5="seed2.bitwebcore.net"
seed_6="seed3.bitwebcore.net"
seed_7=""
seed_8=""
seed_9=""
seed_10=""
...
cf_api_key="YOUR_API_TOKEN_HERE"
...
|-----------|-------|--------|
| `protocol_version` | `70016` | `src/node/protocol_version.h` → `PROTOCOL_VERSION` |
| `init_proto_version` | `209` | `src/node/protocol_version.h` → `INIT_PROTO_VERSION` |
| `min_peer_proto_version` | `31800` | `src/node/protocol_version.h` → `MIN_PEER_PROTO_VERSION` |
| `caddr_time_version` | `31402` | not used |
| `pchMessageStart_0..3` | `0xfe 0xae 0xd5 0xca` | `src/chainparams.cpp` → `pchMessageStart[0..3]` |
| `wallet_port` | `26333` | `src/chainparams.cpp` → `nDefaultPort` |

### Running with Cloudflare (Bitweb Core)

1. Start the seeder in the background (no DNS server needed, Cloudflare handles DNS):

```
cd /opt/generic-seeder
./dnsseed
```

2. Wait for the first `dnsseed.dump` to be written (takes ~100 seconds after first start):

```
watch -n5 wc -l dnsseed.dump
```

3. Push seeds to Cloudflare manually to verify:

```
cd /opt/generic-seeder/cf-uploader
/opt/generic-seeder/cf-venv/bin/python3 seeder.py
```

4. Set up the cron job (replace path if different):
...
```
*/30 * * * * cd /opt/generic-seeder/cf-uploader && /opt/generic-seeder/cf-venv/bin/python3 seeder.py >> /opt/generic-seeder/cf-uploader/seeder.log 2>&1
```

5. Verify DNS records are live:
